...
public class Fractal {
  ...
  private void drawKochCurve(Point2D p1, Point2D p2, int level) {
    ...
    Turtle jerry = new Turtle(p1, p2);
    double distance = p1.distance(p2);
    jerry.move(distance / 3.0);
    Point2D p3 = jerry.getPosition();
    jerry.turnLeft(60.0);
    jerry.move(distance / 3.0);
    Point2D p4 = jerry.getPosition();
    jerry.turnRight(120.0);
    jerry.move(distance / 3.0);
    Point2D p5 = jerry.getPosition();
  ...
  private void drawTree(Point2D p1, Point2D p2, int level) {
    ...
    Turtle freddy = new Turtle(p1, p2);
    double distance = p1.distance(p2);
    freddy.move(distance / 3.0);
    Point2D p3 = freddy.getPosition();
    freddy.turnLeft(60.0);
    freddy.move(2.0/3.0 * distance);
    Point2D p4 = freddy.getPosition();
    freddy.turnLeft(180.0);
    freddy.move(2.0/3.0 * distance);
    freddy.turnLeft(105.0);
    freddy.move(2.0/3.0 * distance);
    Point2D p5 = freddy.getPosition();
  ...
  private void drawSierpinskiTriangle(Point2D p1, Point2D p2, Point2D p3, int level) {
    ...
    double x4 = ( (p1.getX() + p2.getX()) / 2 );
    double y4 = ( (p1.getY() + p2.getY()) / 2 );
    Point2D p4 = new Point2D.Double(x4, y4);
    
    double x5 = ( (p2.getX() + p3.getX()) / 2 );
    double y5 = ( (p2.getY() + p3.getY()) / 2 );
    Point2D p5 = new Point2D.Double(x5, y5);
    
    double x6 = ( (p1.getX() + p3.getX()) / 2 );
    double y6 = ( (p1.getY() + p3.getY()) / 2 );
    Point2D p6 = new Point2D.Double(x6, y6);
  ...
  private void drawSierpinskiCarpet(Point2D p, double a, int level) {
    ...
    double y2 = (Y + side);
    Point2D R2 = new Point2D.Double(X, y2);
    
    double y3 = (y2 + side);
    Point2D R3 = new Point2D.Double(X, y3);
    
    double x4 = (X + side);
    Point2D R4 = new Point2D.Double(x4, y3);
    
    double x5 = (x4 + side);
    Point2D R5 = new Point2D.Double(x5, y3);
    
    
    Point2D R6 = new Point2D.Double(x5, y2);
    
    Point2D R7 = new Point2D.Double(x5, Y);
    Point2D R8 = new Point2D.Double(x4, Y);
    
    drawSierpinskiCarpet(R1 , side, level - 1);
    drawSierpinskiCarpet(R2 , side, level - 1);
    drawSierpinskiCarpet(R3 , side, level - 1);
    drawSierpinskiCarpet(R4 , side, level - 1);
    drawSierpinskiCarpet(R5 , side, level - 1);
    drawSierpinskiCarpet(R6 , side, level - 1);
    drawSierpinskiCarpet(R7 , side, level - 1);
    drawSierpinskiCarpet(R8 , side, level - 1);
